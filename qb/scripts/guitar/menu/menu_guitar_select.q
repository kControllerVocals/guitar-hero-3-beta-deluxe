...
script create_select_guitar_menu player = 1
	guitar_select_available_instruments = [none]
	if (<player> = 1)
	...
	if (<instrument_type> = guitar)
		guitar_select_available_instruments = ($Free_Guitars)
		<secret_instrument_array> = ($Secret_Guitars)
		<download_instrument_array> = Download_Guitars
	else
		guitar_select_available_instruments = ($Free_Basses)
	...
	if GlobalExists name = <download_instrument_array>
		...
		if is_musician_instrument_downloaded index = <index>
			...
			guitar_select_available_instruments = (<array>)
		endif
		<i> = (<i> + 1)
	...
	GetArraySize (<guitar_select_available_instruments>)
	...
	if ($current_num_players = 1)
		0xfa884bde = (<guitar_select_available_instruments> [$guitar_select_highlighted_guitar_p1])
		...
		if (<player> = 1)
			...
			FormatText checksumname = player_status 'player%i_status' i = <player>
			their_instrument_id = ($<player_status>.instrument_id)
			if NOT (<0xfa884bde> = <their_instrument_id>)
				get_instrument_name_and_index id = <their_instrument_id>
				select_guitar_get_available_index_from_total_index index = <instrument_index> gs_available_instruments = <guitar_select_available_instruments>
				if ($is_network_game = 1)
					change online_guitar_highlighted_id_p1 = ($<player_status>.instrument_id)
					change online_old_guitar_id_p1 = ($<player_status>.instrument_id)
				else
					change guitar_select_highlighted_guitar_p1 = <index>
					change guitar_select_old_guitar_p1 = <index>
				endif
			...
			GetScreenElementDims id = $gs_guitar_brand_ID_p1
			if (<width> > 260)
				SetScreenElementProps id = $gs_guitar_brand_ID_p1 scale = (1.0, 0.65000004)
				fit_text_in_rectangle id = $gs_guitar_brand_ID_p1 dims = ((255.0, 0.0) + ((<Height> * 0.65000004) * (0.0, 1.0))) pos = (0.0, 22.0)
			endif
			displayText {
				parent = select_guitar_container
				text = <instrument_finish>
			...
			0xfa884bde = (<guitar_select_available_instruments> [$guitar_select_highlighted_guitar_p2])
			FormatText checksumname = player_status 'player%i_status' i = <player>
			their_instrument_id = ($<player_status>.instrument_id)
			if NOT (<0xfa884bde> = <their_instrument_id>)
				get_instrument_name_and_index id = <their_instrument_id>
				select_guitar_get_available_index_from_total_index index = <instrument_index> gs_available_instruments = <guitar_select_available_instruments>
				if ($is_network_game = 1)
					change online_guitar_highlighted_id_p2 = ($<player_status>.instrument_id)
					change online_old_guitar_id_p2 = ($<player_status>.instrument_id)
				else
			...
			if (<width> > 260)
				SetScreenElementProps id = $gs_guitar_brand_ID_p2 scale = (1.0, 0.65000004)
				fit_text_in_rectangle id = $gs_guitar_brand_ID_p2 dims = ((255.0, 0.0) + ((<Height> * 0.65000004) * (0.0, 1.0))) pos = (0.0, 22.0)
			endif
			displayText {
				parent = select_guitar_container_p2
				text = <instrument_finish>
	...
	if ($current_num_players = 1)
		...
		if CompositeObjectExists name = <char_name>
			<char_name> :handle_change_stance stance = stance_frontend_guitar no_wait
		endif
	endif
	if ($current_num_players = 1)
...
script select_guitar_build_brand_array instrument_type = guitar player = 1
	GetArraySize (<gs_available_instruments>)
	...
	if (<player> = 1)
		if ($gs_made_first_selection_bool_p1 = 0)
			...
			if NOT (<inst_ID> = $<player_status>.instrument_id)
				...
				KillSpawnedScript name = gs_wait_to_load_guitar_p1
				spawnscriptnow gs_wait_to_load_guitar_p1
				if ($current_num_players = 1)
			...
			if (($is_network_game) = 1)
				...
				network_player_lobby_message {
					type = guitar_select
					action = update
					checksum = ($<player_status>.instrument_id)
				}
			endif
			...
			SetScreenElementProps id = $gs_guitar_finish_ID_p1 text = <instrument_finish>
		endif
	else
		if ($gs_made_first_selection_bool_p2 = 0)
			if ($is_network_game = 0)
				...
				change online_guitar_highlighted_id_p2 = <guitar_id>
			endif
			<inst_ID> = <instrument_id>
			FormatText checksumname = player_status 'player%i_status' i = <player>
			if NOT (<inst_ID> = $<player_status>.instrument_id)
				change structurename = <player_status> instrument_id = <inst_ID>
				KillSpawnedScript name = gs_wait_to_load_guitar_p2
				spawnscriptnow gs_wait_to_load_guitar_p2
			endif
			get_instrument_brand_and_finish id = <inst_ID>
			...
			SetScreenElementProps id = $gs_guitar_finish_ID_p2 text = <instrument_finish>
		endif
	endif
endscript
...
script select_guitar_go_back player = 1
	if (<player> = 1)
		...
		if ($is_network_game = 1)
			instrument_id = ($online_old_guitar_id_p2)
		else
...
script 0x138f33d2 
	if ((($is_network_game) = 0) || <player> = 1)
		GetTags
		SetScreenElementProps id = <id> rgba = [125 0 0 255]
	endif
	if (<player> = 1)
		change guitar_select_highlighted_guitar_p1 = <guitar_index>
		get_musician_instrument_struct index = <guitar_index>
		instrument_id = (<info_struct>.desc_id)
		if (($is_network_game) = 1)
			network_player_lobby_message {
				type = guitar_select
				action = update
				value1 = ($guitar_select_highlighted_guitar_p1)
			}
		endif
	else
		change guitar_select_highlighted_guitar_p2 = <guitar_index>
		get_musician_instrument_struct index = <guitar_index>
		instrument_id = (<info_struct>.desc_id)
	endif
	FormatText checksumname = player_status 'player%i_status' i = <player>
...
script select_guitar_choose player = 1
	if (<player> = 1)
		if ($gs_made_first_selection_bool_p1 = 1)
			change structurename = player1_status instrument_id = (<gs_available_instruments> [($guitar_select_highlighted_guitar_p1)])
			change guitar_select_old_guitar_p1 = $guitar_select_highlighted_guitar_p1
			if ($is_network_game = 1)
				network_player_lobby_message {
					type = guitar_select
					action = select
					checksum = ($player1_status.instrument_id)
				}
				change online_old_guitar_id_p1 = ($player1_status.instrument_id)
			endif
			...
			select_guitar_create_finish_menu gs_available_instruments = <gs_available_instruments> player = <player>
			change gs_made_first_selection_bool_p1 = 1
		endif
	else
		if ($gs_made_first_selection_bool_p2 = 1)
			if ($is_network_game = 1)
				change online_old_guitar_id_p2 = <guitar_id>
				change structurename = player2_status instrument_id = <guitar_id>
			else
				change structurename = player2_status instrument_id = (<gs_available_instruments> [($guitar_select_highlighted_guitar_p2)])
			endif
			change guitar_select_old_guitar_p2 = $guitar_select_highlighted_guitar_p2
			ui_flow_manager_respond_to_action action = select_character_hub player = <player> create_params = {player = <player>} destroy_params = {player = <player>}
		else
			if ($is_network_game = 1)
				select_guitar_create_finish_menu player = <player>
			else
				select_guitar_create_finish_menu gs_available_instruments = <gs_available_instruments> player = <player>
			endif
...
script get_instrument_name_and_index 
	...
	<index> = (<index> + 1)
	repeat <array_size>
	return \{instrument_name = "Name not found"
		instrument_index = 0}
endscript
...
script get_instrument_brand_and_finish 
	...
	if (<id> = (<info_struct>.desc_id))
		...
		return instrument_brand = <gitaf_instrument_brand> instrument_finish = <gitaf_instrument_finish>
	endif
	<index> = (<index> + 1)
	repeat <array_size>
...
script select_guitar_change_blurb_text 
	if GotParam inst_ID
...
script select_guitar_scroll_instrument_info scroll_time = 20 z = 2 window_height = 950 x_dims = 362
	<start_pos> = (0.0, 300.0)
	<end_pos> = (0.0, -1000.0)
	<this_id> = 0
	CreateScreenElement {
		...
		z_priority = <z>
		dims = ((0.0, 3000.0) + (<x_dims> * (1.0, 0.0)))
		line_spacing = 1.25
	}
	<this_id> = <id>
	...
	if (<line_nums> < 7)
		SetScreenElementProps id = <this_id> alpha = 0
		doScreenElementMorph id = <this_id> alpha = 1 time = 1.5
		return
	elseif (<line_nums> > 46)
		<end_pos> = (0.0, -1675.0)
	elseif (<line_nums> > 40)
	...
	Wait ((<line_nums> - 6) * 0.82) seconds
	GetScreenElementProps id = <this_id>
	SetScreenElementProps id = <this_id> pos = <pos>
	Wait 3.5 seconds
	doScreenElementMorph id = <this_id> alpha = 0 time = 1.5
	Wait 1.7 seconds
	SetScreenElementProps id = <this_id> alpha = 1 pos = (0.0, 0.0)
	break
	repeat
	repeat
endscript
...
script select_guitar_get_first_guitar_brand_in_list brand = explorer
	...
	GetArraySize (<gs_available_instruments>)
	begin
	if ((<gs_available_instruments> [<j>]) = (<info_struct>.desc_id))
		if ((<info_struct>.name) = <brand>)
			return instrument_id = (<info_struct>.desc_id) instrument_index = <j>
		endif
	endif
	<j> = (<j> + 1)
	repeat <array_size>
...
script select_guitar_create_finish_menu player = 1
	...
	event_handlers = [
		...
		{pad_back select_guitar_go_back_from_finish_menu params = {player = <player>}}
	]
	<p1_menu_pos> = (566.0, 480.0)
	<p2_menu_pos> = (786.0, 490.0)
	<menu_offset> = (46.0, 0.0)
	if (<player> = 1)
		if ($current_num_players = 2)
			new_menu {
				...
				use_backdrop = 0
				event_handlers = <event_handlers>
				menu_pos = (<p1_menu_pos> + (-178.0, 10.0))
				exclusive_device = <exclusive_dev>
	...
	if ($current_num_players = 1)
		...
		if (<player> = 1)
			LaunchEvent type = unfocus target = scrolling_select_guitar_p1
			LaunchEvent type = focus target = <vparent>
			SetScreenElementProps id = select_guitar_container hide
			CreateScreenElement {
				type = ContainerElement
				id = select_finish_container
				parent = root_window
			...
			SetScreenElementProps id = select_guitar_container_p2 hide
			CreateScreenElement {
				type = ContainerElement
				id = select_finish_container_p2
				parent = root_window
				just = [left top]
				pos = (866.0, 500.0)
			}
			displaySprite {
				parent = select_finish_container_p2
				tex = guitar_select_2p_frame
				just = [center center]
				z = 0
			}
			...
			if ($is_network_game = 0)
				...
				get_instrument_brand_and_finish id = ($online_guitar_highlighted_id_p2)
				CreateScreenElement {
					type = TextElement
					parent = <vparent>
					id = 0x2fb4d1b4
...
script select_guitar_go_back_from_finish_menu player = 1
	if ($current_num_players = 1)
		...
		if (<player> = 1)
			SetScreenElementProps id = select_guitar_container unhide
			LaunchEvent type = focus target = scrolling_select_guitar_p1
			LaunchEvent type = unfocus target = scrolling_guitar_finish_menu_p1
			destroy_menu menu_id = scrolling_guitar_finish_menu_p1
			change gs_made_first_selection_bool_p1 = 0
			...
			SetScreenElementProps id = select_guitar_container_p2 unhide
...
script select_guitar_finish_highlight player = 1
	...
	if NOT (<id> = $<player_status>.instrument_id)
		...
		change globalname = <change_flag> newvalue = 1
		if ($current_num_players = 1)
			select_guitar_change_blurb_text inst_ID = <id>
		endif
	endif
	if ($is_network_game = 1)
		if (<player> = 1)
			...
			0x2fb4d1b4 :SetProps text = <instrument_finish>
		endif
	endif
	get_instrument_brand_and_finish id = <id>
	if ($current_num_players = 1)
