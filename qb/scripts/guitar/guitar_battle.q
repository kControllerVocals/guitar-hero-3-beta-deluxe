...
script battlemode_ready battle_gem = 0 player_status = player1_status steal = 0 battle_text = 1
	...
	if (<current_num_powerups> < 0)
		...
		printf "playerstatus=%p battle_gem=%b" p = <player_status> b = <battle_gem>
		printf "Current powerups p1:"
		GetArraySize $current_powerups_p1
		array_count = 0
		begin
		printf "    %c: %p" c = <array_count> p = ($current_num_powerups [<array_count>])
		repeat <array_size>
		printf "Current powerups p2:"
...
script battle_whammy_attack 
	...
	if ($<other_player_status>.whammy_attack < 1)
		...
		if ($<other_player_status>.is_local_client)
			...
			GuitarGetAnalogueInfo controller = ($<other_player_status>.controller)
			...
			net_whammy_attack player_text = <player_text> other_player_status = <other_player_status>
		endif
...
script battle_broken_string 
	...
	if NOT (<victim_is_local>)
		Block type = 0xdff720de
		x = (<event_data>.string)
	else
		GetArraySize $gem_colors
		gem_color = 0
	...
	if (($is_network_game) && (<net_update_flags>))
