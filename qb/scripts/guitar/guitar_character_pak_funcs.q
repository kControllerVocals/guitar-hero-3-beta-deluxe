...
script get_pak_filename 
	...
	if (<found> = 0)
		printstruct <...>
		printscriptinfo "Pak filename not found"
	endif
...
script load_musician_pak_file startslot = 0
	...
	if (<found> = 0)
		return filename_crc = no_pak_id assetcontext = 0
	endif
	...
	if (<found> = 1)
		...
		get_pak_filename desc_id = <desc_id> type = <type>
		if (<found> = 1)
			LoadPakAsync pak_name = <pak_name> heap = ($<loaded_heaps> [<index>]) async = <async>
		endif
	endif
	return filename_crc = <filename_crc> assetcontext = <assetcontext>
endscript
...
script LoadPakAsync 
	...
	if (<async> = 0)
		if (GotParam no_vram)
			LoadPak <pak_name> heap = <heap> device = <device> no_vram
		else
			LoadPak <pak_name> heap = <heap> device = <device>
		endif
